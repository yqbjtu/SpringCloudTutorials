...
public class UserServiceClientFallbackFactory implements UserServiceClient{
    ...
    @Override
    public String getUserDetail(String userId, Long sleepTimeMillis) {
        log.error("Fallback2 sleep, userId={}", userId);
        return "user-service not available2 when query '" + userId + "'," + sleepTimeMillis;
    }
}